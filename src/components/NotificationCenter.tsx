...
import {
  ...
  AlertTriangle, Sparkles, Users, CreditCard, Info, Loader2,
} from 'lucide-react'
...
const TYPE_CONFIG: Record<string, { icon: typeof Bell; color: string; bg: string; border: string }> = {
  query_complete:  { icon: Sparkles,      color: 'text-violet-500',       bg: 'bg-violet-500/10',       border: 'border-violet-500/20'      },
  threshold_alert: { icon: AlertTriangle, color: 'text-amber-500',        bg: 'bg-amber-500/10',        border: 'border-amber-500/20'       },
  schedule_failed: { icon: AlertTriangle, color: 'text-destructive',      bg: 'bg-destructive/10',      border: 'border-destructive/20'     },
  team_invite:     { icon: Users,         color: 'text-blue-500',         bg: 'bg-blue-500/10',         border: 'border-blue-500/20'        },
  billing:         { icon: CreditCard,    color: 'text-emerald-500',      bg: 'bg-emerald-500/10',      border: 'border-emerald-500/20'     },
...
export function NotificationCenter() {
  ...
  return (
    <div className="relative">
      ...
      <button
        ref={buttonRef}
        onClick={() => { setIsOpen(o => !o); if (!isOpen) loadNotifications() }}
        className={clsx(
      ...
      {isOpen && (
        <div
          ref={panelRef}
          className="absolute right-0 top-full mt-2 w-[400px] bg-card border border-border rounded-2xl shadow-2xl z-50 overflow-hidden animate-scaleIn"
        >
            ...
            {isLoading && notifications.length === 0 ? (
              ...
              <div className="flex flex-col items-center justify-center py-14 px-8 text-center">
                ...
                <p className="text-xs text-muted-foreground leading-relaxed max-w-[220px]">
                  You'll be notified when you reach query limits, scheduled queries run, and for team activity.
                </p>
                {/* Decorative pills */}
                <div className="flex items-center gap-2 mt-5 flex-wrap justify-center">
                  {[
                    { icon: AlertTriangle, label: 'Usage alerts', color: 'text-amber-500 bg-amber-500/10' },
                    { icon: Sparkles,      label: 'Scheduled queries', color: 'text-violet-500 bg-violet-500/10' },
                    { icon: Users,         label: 'Team updates', color: 'text-blue-500 bg-blue-500/10' },
                  ].map(({ icon: Icon, label, color }) => (
                    <span key={label} className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-medium border border-border/50 ${color}`}>
              ...
              <div className="divide-y divide-border/40">
                {notifications.map(n => {
                  ...
                  return (
                    ...
                    >
                      <div className={clsx('w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 mt-0.5 border', cfg.bg, cfg.border)}>
                        <Icon className={clsx('w-4 h-4', cfg.color)} />
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2">
                          <p className={clsx('text-sm leading-snug', !n.read ? 'font-semibold' : 'font-medium')}>
                            {n.title}
                          </p>
                          {!n.read && (
