...
export function WorkspaceHeader({
  ...
  const [shareStatus, setShareStatus] = useState<{ isPublic: boolean; token?: string }>({
    isPublic: false,
  })
  ...
  const handleToggleShare = async (isPublic: boolean) => {
    ...
    if (res.success) {
      ...
      toast.success(
        isPublic
          ? t('dashboard.header.dashboardIsPublic')
          : t('dashboard.header.dashboardIsPrivate')
      )
    } else {
      ...
      <header className="relative z-20 border-b border-border bg-card/50 backdrop-blur-sm px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs sm:text-sm text-muted-foreground truncate">
              {activeConnection
                ? t('dashboard.header.connectedTo', { name: activeConnection.name })
                : t('dashboard.header.selectConnection')}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
          ...
          <button
            ...
            className={clsx('tour-step-3 btn-ghost text-sm', showHistory && 'bg-secondary')}
          >
          ...
          {onOpenScheduler && (
            <button
              ...
              title={t('dashboard.scheduler.title')}
            >
              <Clock className="w-4 h-4" />
              <span className="hidden sm:inline">{t('dashboard.scheduler.title')}</span>
              {(scheduledCount ?? 0) > 0 && (
                <span className="ml-1 px-1.5 py-0.5 text-xs bg-primary/10 text-primary rounded-full">
                  {scheduledCount}
                </span>
            ...
            {shareStatus.isPublic && shareStatus.token ? (
              <div className="mb-6 animate-fadeIn">
                <label className="text-xs font-semibold text-muted-foreground mb-1 block">
                  {t('dashboard.header.publicDashboardLink')}
                </label>
