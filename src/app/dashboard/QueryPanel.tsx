...
interface QueryPanelProps {
  ...
  onPinToDashboard?: (question: string, sql: string, data: QueryResultRow[], fields: string[]) => void
...
export function QueryPanel({
  ...
  const handleToggleSuggestions = () => {
    setSuggestionsEnabled(prev => {
      const next = !prev
      try { localStorage.setItem('rf_suggestions_enabled', String(next)) } catch {}
      return next
    })
  }
        ...
        <div className="max-w-2xl w-full">
          <div className="card rounded-2xl p-10 md:p-14 text-center border border-border/50 shadow-2xl relative overflow-hidden group">
            ...
            <h2 className="text-2xl font-bold mb-3">
              {t('dashboard.queryPanel.connectDatabase')}
            </h2>
    ...
    <div className="flex-1 min-w-0 overflow-y-auto px-6 pb-6 pt-2">
      <div className="max-w-6xl mx-auto space-y-5">
            ...
            <>
              <div className="flex items-center gap-4 mb-6">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-muted-foreground">{t('dashboard.queryPanel.describeData')}</p>
                </div>
              </div>
              ...
              {conversationLength && conversationLength > 0 ? (
                <div className="flex items-center gap-2 mb-3 px-3 py-1.5 bg-primary/5 rounded-lg border border-primary/10">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {t('dashboard.queryPanel.conversation')}: {conversationLength} {conversationLength === 1 ? t('dashboard.queryPanel.conversationExchange') : t('dashboard.queryPanel.conversationExchanges')}
                  </span>
                  <button
              ...
              {schemaSuggestions && (
                <div className="mt-3 p-3 bg-accent/5 border border-accent/10 rounded-lg">
                  <p className="text-xs font-medium text-accent mb-2 flex items-center gap-1.5">
                    ...
                    {isDiscovering ? t('dashboard.queryPanel.analyzingSchema') : t('dashboard.queryPanel.schemaInsights')}
                  </p>
              ...
              <div className="flex items-center justify-between mt-4">
                ...
                <span className="text-xs text-muted-foreground hidden sm:inline">
                  Press <kbd className="px-1.5 py-0.5 bg-secondary border border-border rounded text-xs">Ctrl</kbd> + <kbd className="px-1.5 py-0.5 bg-secondary border border-border rounded text-xs">Enter</kbd>
                </span>
              </div>
            </>
          )}
            ...
            <>
              <div className="flex items-center gap-4 mb-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-muted-foreground">{t('dashboard.queryPanel.sqlEditorDescription')}</p>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between mt-4">
                <div className="flex items-center gap-2">
                  ...
                  >
                    ...
                    {isExecuting ? t('dashboard.queryPanel.running') : t('dashboard.queryPanel.runQuery')}
                  </button>
                  <button
                    onClick={() => {
                      try {
                ...
                <span className="text-xs text-muted-foreground hidden sm:inline">
                  <kbd className="px-1.5 py-0.5 bg-secondary border border-border rounded text-xs">Ctrl</kbd> + <kbd className="px-1.5 py-0.5 bg-secondary border border-border rounded text-xs">Enter</kbd> to run
                  &nbsp;·&nbsp;
                  <kbd className="px-1.5 py-0.5 bg-secondary border border-border rounded text-xs">Ctrl</kbd> + <kbd className="px-1.5 py-0.5 bg-secondary border border-border rounded text-xs">Space</kbd> for autocomplete
                </span>
              </div>
            </>
          )}
        ...
        {error && (() => {
          ...
          return (
            <div className={`border rounded-xl p-4 flex items-start justify-between gap-3 animate-slideUp ${isGroqLimit ? 'bg-amber-500/10 border-amber-500/30' : 'bg-destructive/10 border-destructive/30'}`}>
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h4 className={`font-medium ${isGroqLimit ? 'text-amber-600 dark:text-amber-400' : 'text-destructive'}`}>
                    {isGroqLimit ? 'Groq API Limit (External Service)' : t('common.error')}
                  </h4>
                  <p className={`text-sm mt-1 ${isGroqLimit ? 'text-amber-600/80 dark:text-amber-400/80' : 'text-destructive/80'}`}>
                    {error}
                  </p>
                  {isGroqLimit && (
                    <p className="text-xs text-muted-foreground mt-1.5">
                      This is Groq&apos;s server-side limit, not ours. Use the <strong>SQL Editor</strong> tab to run queries directly while waiting.
                    </p>
                  )}
                </div>
              </div>
          ...
          <div className="card p-5 animate-slideUp">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">{t('dashboard.queryPanel.readyToExecute')}</p>
                </div>
              </div>
              <button
            ...
            <div className="flex items-center justify-between mt-3">
              ...
              <div className="flex items-center gap-3">
                ...
                >
                  ...
                  {isExplaining ? t('dashboard.queryPanel.explaining') : explanation ? t('dashboard.queryPanel.reExplain') : t('dashboard.queryPanel.explainQuery')}
                </button>
              </div>
            </div>
            ...
            {explanation && (
              <div className="mt-3 p-3 bg-primary/5 border border-primary/10 rounded-lg">
                <p className="text-xs font-medium text-primary mb-1.5">{t('dashboard.queryPanel.whatQueryDoes')}</p>
        ...
        {/* Query Results */}
        {queryResults && (
      <GlowCard key={queryResults.rowCount + '-' + queryResults.executionTime} className="p-5 animate-slideUp">
            {/* Results Header */}
              ...
              <div className="flex items-center gap-1">
                ...
                <button
                  ...
                  className={clsx(
                    ...
                    resultsView === 'table' ? 'bg-primary/10 text-primary' : 'text-muted-foreground hover:text-foreground hover:bg-secondary'
                  )}
                  title="Table view"
                >
                ...
                <button
                  ...
                  className={clsx(
                    'p-2 rounded-lg transition-colors',
                    resultsView === 'chart' ? 'bg-primary/10 text-primary' : 'text-muted-foreground hover:text-foreground hover:bg-secondary'
                  )}
                  title="Chart view"
                >
                ...
                <button
                  ...
                  className={clsx(
                    'p-2 rounded-lg transition-colors',
                    resultsView === 'insights' ? 'bg-primary/10 text-primary' : 'text-muted-foreground hover:text-foreground hover:bg-secondary'
                  )}
                  title="AI Insights"
                >
                ...
                <button
                  ...
                  className={clsx(
                    'p-2 rounded-lg transition-colors',
                    resultsView === 'performance' ? 'bg-primary/10 text-primary' : 'text-muted-foreground hover:text-foreground hover:bg-secondary'
                  )}
                  title="Performance"
                >
                ...
                {onPinToDashboard && queryResults.rows.length > 0 && (
                  <button
                    onClick={() => onPinToDashboard(nlQuery, generatedSQL, queryResults.rows, queryResults.fields)}
            ...
            {/* Results Content */}
            {resultsView === 'table' && (
              <ResultsTable rows={queryResults.rows} fields={queryResults.fields} rowCount={queryResults.rowCount} executionTime={queryResults.executionTime} />
            )}
            {resultsView === 'chart' && (
              <ResultsChart 
