...
export function BillingSettings() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-lg font-semibold">{t('settings.billing.title')} &amp; {t('common.settings')}</h2>
      ...
      <div className="card p-5">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              {sub.status === 'TRIALING' && sub.trialEndsAt ? (
                (() => {
                  const daysLeft = Math.max(0, Math.ceil((new Date(sub.trialEndsAt).getTime() - Date.now()) / (1000 * 60 * 60 * 24)))
                ...
                : sub.currentPeriodEnd
                  ? t('settings.billing.renews', { date: new Date(sub.currentPeriodEnd).toLocaleDateString() })
        ...
        <div className="mt-5 grid grid-cols-3 gap-3">
          <div className="glass-card rounded-xl p-4 text-center">
            <p className="text-xs text-muted-foreground mb-1">{t('settings.billing.queriesPerMonth')}</p>
            ...
            {usage && sub.limits.queriesPerMonth !== -1 && (
              <div className="mt-2 h-1.5 bg-secondary rounded-full overflow-hidden">
                <div
                  className={`h-full rounded-full transition-all ${usage.queries.current / sub.limits.queriesPerMonth > 0.9
                      ? 'bg-destructive'
                      : usage.queries.current / sub.limits.queriesPerMonth > 0.7
                        ? 'bg-warning'
                        : 'bg-primary'
                    }`}
                  style={{ width: `${Math.min(100, (usage.queries.current / sub.limits.queriesPerMonth) * 100)}%` }}
                />
              </div>
            )}
          ...
          <div className="glass-card rounded-xl p-4 text-center">
            <p className="text-xs text-muted-foreground mb-1">{t('settings.billing.connections')}</p>
            <p className="text-xl font-bold stat-number">
            ...
            {usage && sub.limits.connections !== -1 && (
              <div className="mt-2 h-1.5 bg-secondary rounded-full overflow-hidden">
                <div
                  className={`h-full rounded-full transition-all ${usage.connections.current / sub.limits.connections >= 1
                      ? 'bg-destructive'
                      : 'bg-primary'
                    }`}
                  style={{ width: `${Math.min(100, (usage.connections.current / sub.limits.connections) * 100)}%` }}
                />
              </div>
            )}
          </div>
          <div className="glass-card rounded-xl p-4 text-center">
            <p className="text-xs text-muted-foreground mb-1">{t('settings.billing.teamMembers')}</p>
            <p className="text-xl font-bold stat-number">
            ...
            {usage && sub.limits.teamMembers > 0 && (
              <div className="mt-2 h-1.5 bg-secondary rounded-full overflow-hidden">
                <div
                  className={`h-full rounded-full transition-all ${usage.teamMembers.current / sub.limits.teamMembers >= 1
                      ? 'bg-destructive'
                      : 'bg-primary'
                    }`}
                  style={{ width: `${Math.min(100, (usage.teamMembers.current / sub.limits.teamMembers) * 100)}%` }}
                />
              </div>
            )}
          </div>
      ...
      <div className="grid md:grid-cols-3 gap-4 stagger-children">
        {(['FREE', 'PRO', 'ENTERPRISE'] as const).map((planKey) => {
          ...
          return (
            <div
              key={planKey}
              className={`relative p-5 flex flex-col rounded-xl transition-all duration-300 ${isCurrent
            ...
            >
              ...
              <div className="flex items-center gap-3 mb-4">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${isPro ? 'bg-gradient-to-br from-primary to-accent' :
                  isEnterprise ? 'bg-gradient-to-br from-accent to-primary' :
                    'bg-primary/10'
                  }`}>
                  <Icon className={`w-5 h-5 ${isPro || isEnterprise ? 'text-white' : 'text-primary'}`} />
                </div>
                <div>
              ...
              {isCurrent ? (
                <button
                  className="btn-secondary w-full text-sm opacity-60 cursor-default"
                  disabled
                >
